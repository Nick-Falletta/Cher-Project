...
const Complete = ({ coins, setCoins, answers }) => {
  ...
  return (
    <div className="flex flex-col bg-gray-50 min-h-screen text-gray-800 pb-20 lg:pb-0">
      {/* Confetti explosion when coin counter finishes */}
      ...
      {/* Title Completion */}
      <div className="text-3xl font-bold mb-3">{module1.title} <span className='text-gray-50 bg-gray-800 rounded px-1'>Completed</span></div>
      {/* Received Coins Text */}
      <div className='flex flex-row text-xl'>
        <div className="flex mr-1">You have received</div> 
        <b className='text-orange-600 border-b-2 flex-row flex '>{tempCoins}
      ...
      <div className='mb-4 flex justify-center flex-col lg:flex-row'>
        {module1.lessons.map((lesson, idx) => {
          ...
          return (
            <div key={idx} className="mb-4 border border-gray-300 rounded-lg p-4 bg-white shadow-sm lg:w-100 lg:mr-6">
